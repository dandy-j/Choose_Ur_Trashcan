...
public class TouchIt extends BaseGameActivity implements IOnSceneTouchListener {
	...
	private Font mFont;
	private ChangeableText score,wins;
	...
	public static int quantity,level;
...
private void createSpriteSpawnTimeHandler() {//fungsi yang dijalankan setiap satuan waktu
    ...
    new ITimerCallback() {
        ...
        public void onTimePassed(TimerHandler pTimerHandler) {
        	...
        	if (win==true){
        		skor=tex;
        		//Intent setskor = new Intent(TouchIt.this,MainActivity.class);
        		//startActivity(setskor);
        		score.setText("YOU WIN IN "+tex);
        		score.setPosition(mCamera.getCenterX()-score.getWidth()/2, mCamera.getCenterY());
        	}
        	if(win==false){
        	mem++;
            count++;
...
public static void level(int lev){//fungsi pemilihan level
	level=lev;
	quantity = 9*level;
}
public void cek(int t){//fungsi pengecekan kondisi win
	if(t==random*10){
		if(con<quantity){
		randomize();
		con=0;
...
public void randomcan(){//merubah posisi tempat sampah pada level 6-9
	r++;
	if(r>3){
		r=0;
	}
...
public void randomize(){//mengubah posisi objek sampah
...
public void addobj(){//menambahkan objek ke dalam layar game
	...
	@Override
	public Engine onLoadEngine() {//setting layar
	...
	@Override
	public boolean onSceneTouchEvent(Scene pScene, TouchEvent pSceneTouchEvent) {//fungsi ketika layar disentuh
